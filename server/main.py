...
async def get_files(request: Request):
    ...
    try:
        ...
        files = await database.files.find({"user_id": str(user["_id"])}).to_list(length=100)
        return {"files": files}
    except Exception as e:
...
async def get_templates(request: Request):
    ...
    try:
        ...
        templates = await database.templates.find({"user_id": str(user["_id"])}).to_list(length=100)
